...
<body>
	...
	<div id="search_wrap">
		...
		<div id="list_wrap">
					...
					<c:if test="${finds.size() != 0 }">
						<c:forEach items="${finds}" var="f">
							<tr>
								...
								<td>${f.fOb }</td>
								<td class="title"><a
									href="${conPath }/findDetailContent.laf?fNo=${f.fNo}&pageNum=${currentPage}">${f.fTitle }</a>
			...
			<div class="paging">
				<a
					href="${conPath }/findDetailList.laf?pageNum=${startPage - 1 eq 0 ? 1 : startPage-1}&fOb=${fOb}&fLocal=${fLocal}&tempDate1=${tempDate1}&tempDate2=${tempDate2}&fTitle=${fTitle}"><img
				...
				<c:forEach var="j" begin="${startPage }" end="${endPage }">
					...
					<c:if test="${j != currentPage }">
						<a href="${conPath }/findDetailList.laf?pageNum=${j}&fOb=${fOb}&fLocal=${fLocal}&tempDate1=${tempDate1}&tempDate2=${tempDate2}&fTitle=${fTitle}"> ${j } </a>
					</c:if>
				</c:forEach>
				<a
					href="${conPath }/findDetailList.laf?pageNum=${endPage == pageCnt ? endPage : endPage + 1 }&fOb=${fOb}&fLocal=${fLocal}&tempDate1=${tempDate1}&tempDate2=${tempDate2}&fTitle=${fTitle}"><img
