...
-- 권한 변경
UPDATE LAF_MEMBER SET PWCODE = 'PW10' WHERE MID = 'admin01';

-- 사용 중단 처리
UPDATE LAF_MEMBER SET MSTCODE = 'MST10' WHERE MID = 'aaa3';
...
-- 관리자 리스트
SELECT A.*, PC.CODENAME PCC, MC.CODENAME MCC FROM (SELECT * FROM LAF_MEMBER WHERE PWCODE = 'PW10' OR PWCODE = 'PW01') A, PW_CODE PC, MST_CODE MC
...
-- 권한 변경
UPDATE LAF_MEMBER SET PWCODE = 'PW00' WHERE MID = 'admin00';

SELECT * FROM LAF_MEMBER;
...
INSERT INTO FIND (FNO, FTITLE, FCONTENT, MID, FOB, FLOCAL, FLOCATION, FDATE, FSTORAGE, FPHOTO, FTEL, FIP)
    VALUES ('F'||TRIM(TO_CHAR(SYSDATE,'YYYYMMDD'))||TRIM(TO_CHAR(FIND_SEQ.NEXTVAL, '000000')), '구찌 지갑', '현금은 없으며 카드지갑입니다', 'aaa01', '지갑', '경기도',
                '평촌 이마트', '2022-06-20', '평촌 이마트 고객센터', NULL, '031-123-1234', '127.0.0.1');
...
SELECT COUNT(*) CNT FROM (SELECT * 
    FROM (SELECT ROWNUM RN, A.*
            FROM (SELECT F.*, MNAME, CODENAME FCC
                    ...
                    WHERE F.FOB LIKE '%'||'지갑'||'%' AND F.FTITLE LIKE '%'||''||'%' 
                        AND F.FDATE BETWEEN '2022-06-01' AND '2022-06-30'
                        AND F.FLOCAL = '경기도' AND NOT F.FSTCODE = 'FST02'
                        AND F.MID = M.MID AND F.FSTCODE = FC.FSTCODE ORDER BY FDATE) A));
    
SELECT * FROM FIND WHERE FOB = '지갑' AND FTITLE LIKE '%'||'구찌'||'%' AND FDATE BETWEEN '2022-06-20' AND '2022-06-27' AND FLOCAL = '경기도' AND NOT FSTCODE = 'FST02' ORDER BY FDATE;
...
-- QNA 게시글 등록
INSERT INTO QNA (QNO, QTITLE, QCONTENT, MID, QGROUP, QSTEP, QINDENT, QIP)
    VALUES (QNA_SEQ.NEXTVAL, '문의드립니다', '답변부탁드립니다', 'aaa01', QNA_SEQ.CURRVAL, 0, 0, '127.0.0.1');

-- QNA 게시글 수정
UPDATE QNA SET QTITLE = '수정)문의드립니다', QCONTENT = '다시 문의드리며 답변부탁드립니다' WHERE QNO = '1';
...
-- QNA 게시글 답변 등록
INSERT INTO QNA (QNO, QTITLE, QCONTENT, MID, QGROUP, QSTEP, QINDENT, QIP)
    VALUES (QNA_SEQ.NEXTVAL, '답변드립니다', '아시겠죠?', 'admin01', 1, 1, 1, '127.0.0.1');
